...
import { ControllersFilePaths } from "../../types/paths/index.js";
...
export const CONTROLLERS = Object.freeze({
  ...
  EXAMPLE_LOC: (destination: string): string =>
    path.join(
      ...
      PATHS.DIRECTORIES.CONTROLLERS.example,
      CONTROLLERS.EXAMPLE + EXTENSIONS.TS,
    ),
  ...
  EXAMPLE_TEMPLATE_LOC: (): string =>
    path.join(
      PATHS.DIRECTORIES.SRC.CONTROLLERS,
      PATHS.DIRECTORIES.CONTROLLERS.example,
      CONTROLLERS.EXAMPLE_TEMPLATE + EXTENSIONS.AST,
    ),
  ...
  INDEX_TEMPLATE_LOC: (base: boolean): string => {
    ...
    return base
      ? path.join(
          basePath,
          PATHS.DIRECTORIES.CONTROLLERS.example,
          CONTROLLERS.INDEX_TEMPLATE + EXTENSIONS.AST,
        )
