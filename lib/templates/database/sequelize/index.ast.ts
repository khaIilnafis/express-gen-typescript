 ...
 */
export default function generateSequelizeConfigAST(options: GeneratorOptions) {
  // Default values
  ...
  const imports = [
    ...
    b.importDeclaration([], b.stringLiteral("dotenv/config")),
  ];
  ...
  const sequelizeDeclaration = b.variableDeclaration("const", [
    b.variableDeclarator(
      ...
      b.newExpression(b.identifier("Sequelize"), [
        b.objectExpression([
          // host
          b.objectProperty(
            b.identifier("host"),
          ...
          // dialect
          b.objectProperty(b.identifier("dialect"), b.stringLiteral(dialect)),
          // logging
          b.objectProperty(
  ...
  initFunction.returnType = b.tsTypeAnnotation(
    b.tsTypeReference(
      ...
      b.tsTypeParameterInstantiation([b.tsTypeReference(b.identifier("void"))]),
    ),
  );
  initFunction.async = true;
  // Export default sequelize
  const defaultExport = b.exportDefaultDeclaration(b.identifier("sequelize"));
  const exportInitialize = b.exportNamedDeclaration(initializeFunction, []);

  exportInitialize.comments = [
    b.commentBlock(COMMENTS.DATABASE.INITIALIZE_DATBASE, true),
  ];
