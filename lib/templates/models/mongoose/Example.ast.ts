 ...
 */
export default function generateMongooseExampleModelAST(options: TemplateOptions = {}) {
  ...
  const interfaceBody = [
    ...
    b.tsPropertySignature(
      ...
      b.tsTypeAnnotation(
        b.tsTypeReference(b.identifier("Date"))
      )
    ),
  ...
  (interfaceDeclaration as any).extends = [
  ...
  const schemaExpression = b.newExpression(
    b.identifier("Schema"),
    [
  ...
  (schemaExpression as any).typeParameters = b.tsTypeParameterInstantiation([
  ...
  const modelCall = (modelExport.declaration as any).declarations[0].init;
